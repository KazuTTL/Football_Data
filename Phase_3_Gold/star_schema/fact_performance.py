...
def build_fact_performance(output_dir, dim_player, dim_team, dim_position):
    """
    Tạo bảng Fact:
    - Truy vấn metrics từ bảng silver_players trên DWH.
    ...
    # 1. Join player_key
    df_fact = pd.merge(df_fact, dim_player[dim_player["is_current"]==True][["internal_player_id", "player_key"]], 
    ...
    if "sub_position_tm" in df_fact.columns:
        df_fact = pd.merge(df_fact, dim_position, left_on="sub_position_tm", right_on="name", how="left", suffixes=("", "_pos"))
    else:
    ...
    if not df_rating.empty and "internal_player_id" in df_rating.columns and "final_scout_score" in df_rating.columns:
        df_fact = pd.merge(df_fact, df_rating[["internal_player_id", "final_scout_score"]], on="internal_player_id", how="left")
    else:
        df_fact["final_scout_score"] = 0.0

    # 5. Lọc các cột chuẩn cho bảng fact
