...
def render_comparison_tab(fdf, df_rating, theme_config):
        ...
        for label, pname, color, icon_name in [
            ...
            if not ps.empty:
                ...
                if comp_mode == "UEFA Champions League (UCL)":
                    ...
                    card_html = f"<div class='metric-card' style='margin-top:10px;text-align:left;border-left:3px solid {color}; align-items: flex-start; padding: 12px 16px;'><div style='display:flex;align-items:center;font-weight:700;color:{color};margin-bottom:4px;'>{icon_html} {label}: {pname}</div><div style='color:{theme_config['TEXT_SUB']};font-size:0.8rem'>{ps.get('position','?')} · {ps.get('team_cl', ps.get('team','?'))}</div><div style='margin-top:4px;font-size:0.85rem'>UCL Rating: <b style='color:{color}'>{rating_str}</b> &nbsp;|&nbsp; Giá trị: {mv_str}</div><div style='font-size:0.8rem;color:{theme_config['TEXT_SUB']}'>Hiệu suất UCL: {int(ps.get('goals_cl',0))} bàn &nbsp;|&nbsp; {int(ps.get('assists_cl',0))} kiến tạo</div></div>"
                else:
                    pr = df_rating[df_rating["name"] == pname]
                    score_val = pr.iloc[0]['final_scout_score'] if not pr.empty else 0.0
                    card_html = f"<div class='metric-card' style='margin-top:10px;text-align:left;border-left:3px solid {color}; align-items: flex-start; padding: 12px 16px;'><div style='display:flex;align-items:center;font-weight:700;color:{color};margin-bottom:4px;'>{icon_html} {label}: {pname}</div><div style='color:{theme_config['TEXT_SUB']};font-size:0.8rem'>{ps.get('position','?')} · {ps.get('team','?')}</div><div style='margin-top:4px;font-size:0.85rem'>Điểm Scout: <b style='color:{color}'>{score_val:.2f}</b> &nbsp;|&nbsp; Giá trị: {mv_str}</div><div style='font-size:0.8rem;color:{theme_config['TEXT_SUB']}'>Hiệu suất: {int(ps.get('goals',0))} bàn &nbsp;|&nbsp; {int(ps.get('assists',0))} kiến tạo</div></div>"
    ...
    table_html += "</table>"
    st.markdown(f"<div style='background: {theme_config['SURFACE2']}; padding: 16px; border-radius: 10px; border: 1px solid {theme_config['BORDER']}; overflow-x: auto;'>{table_html}</div>", unsafe_allow_html=True)
