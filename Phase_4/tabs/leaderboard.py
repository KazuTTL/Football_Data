...
def render_leaderboard_tab(fdf, theme_config):
    ...
    with sort_col:
        sort_by = st.selectbox("Tiêu chí Sắp xếp ưu tiên:", ["Điểm Scout" if "Điểm Scout" else "Rating", "Bàn thắng", "Kiến tạo", "Giá chuyển nhượng"])
    ...
    if stat_mode == "UEFA Champions League (UCL)":
        ...
        # Adjust sort_by mapping if "Điểm Scout" is selected since UCL uses Rating
        leaderboard_sort_by = "Điểm Scout" if sort_by == "Điểm Scout" else sort_by
        show_df = get_sorted_leaderboard(ucl_fdf, leaderboard_sort_by)
